#include "TP_MGFCameraMode_ThirdPerson.h"
...
#include "Player/Camera/CameraMode/TP_MGFPenetrationAvoidanceFeeler.h"


#include UE_INLINE_GENERATED_CPP_BY_NAME(TP_MGFCameraMode_ThirdPerson)

namespace TP_MGFCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UTP_MGFCameraMode_ThirdPerson::UTP_MGFCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FTP_MGFPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UTP_MGFCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UTP_MGFCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UTP_MGFCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UTP_MGFCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	ITP_MGFCameraAssistInterface* TargetControllerAssist = Cast<ITP_MGFCameraAssistInterface>(TargetController);

	ITP_MGFCameraAssistInterface* TargetActorAssist = Cast<ITP_MGFCameraAssistInterface>(TargetActor);
	...
	ITP_MGFCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<ITP_MGFCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		ITP_MGFCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (ITP_MGFCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UTP_MGFCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO ITP_MGFCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FTP_MGFPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(TP_MGFCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UTP_MGFCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UTP_MGFCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
