...
#define LOCTEXT_NAMESPACE "TP_MGFGame"

UGameDefinition::UGameDefinition()
{

}

#if WITH_EDITOR
EDataValidationResult UGameDefinition::IsDataValid(class FDataValidationContext& Context) const
{
	...
	if (!GetClass()->IsNative())
	{
		...
		{
			Context.AddError(FText::Format(LOCTEXT("GameDescriptionInheritenceIsUnsupported", "Blueprint subclasses of Blueprint game definition is not currently supported (use composition via ActionSets instead). Parent class was {0} but should be {1}."),
...
void UGameDefinition::UpdateAssetBundleData()
{
