...
#include "TP_MGFAudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FTP_MGFAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 * TP_MGF Audio Settings.
 */
UCLASS()
class TP_MGF_API UTP_MGFAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the TP_MGF Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the TP_MGF Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the TP_MGF Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the TP_MGF Audio Settings and linked to the UI and game settings in TP_MGFSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the TP_MGF Audio Settings and linked to the UI and game settings in TP_MGFSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the TP_MGF Audio Settings and linked to the UI and game settings in TP_MGFSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the TP_MGF Audio Settings and linked to the UI and game settings in TP_MGFSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// Submix Effect Chain Overrides to apply when HDR Audio is turned on
	UPROPERTY(Transient)
	TArray<FTP_MGFAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FTP_MGFAudioSubmixEffectsChain> LDRSubmixEffectChain;
